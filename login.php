...
if(isset($_POST['submit'])){
   ...
   $pass = sha1($_POST['pass']); //sử dụng thuật toán Sha1 để mã hoá mật khẩu người dùng
   $pass = filter_var($pass, FILTER_SANITIZE_STRING);

   // truy vấn và lọc dữ liệu 
   $select_user = $conn->prepare("SELECT * FROM `users` WHERE email = ? AND password = ?"); // câu truy vấn
   $select_user->execute([$email, $pass]); //lọc email và password
   $row = $select_user->fetch(PDO::FETCH_ASSOC); // nếu có dữ liệu trùng thì sẽ 
...
<section class="form-container">
   ...
   <form action="" method="post">
      ...
      <p>Bạn chưa có tài khoản? <a href="register.php">Đăng ký ở đây nhé</a></p>
   </form>
